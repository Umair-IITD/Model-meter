...
**Live URL:** [https://model-meter.vercel.app](https://model-meter.vercel.app)
...
4. A personalized AI summary (Anthropic Haiku) appears, with a template fallback
...
- Anthropic API key (for AI summaries; fallback works without it)
...
| `ANTHROPIC_API_KEY` | AI summary generation (optional — template fallback works) |
...
### 4. claude-haiku-4-5 for AI summaries

100-word summaries don't require Sonnet-level reasoning. Haiku is 5x cheaper and significantly faster. If the API fails, the template fallback produces equally useful output — so the AI is additive, not load-bearing.
